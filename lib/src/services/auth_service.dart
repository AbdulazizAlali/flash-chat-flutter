...
class AuthService {
  ...
  Future<void> register(
    ...
    StorageReference firebaseStorageRef =
        _firebaseStorage.ref().child(_getUserImagePath(authResult.user.uid));
  ...
  Future<void> updateProfile(ProfileModel profileData) async {
    ...
    if (profileData.imageFile != null) {
      StorageReference firebaseStorageRef =
          _firebaseStorage.ref().child(_getUserImagePath(currentUser.uid));
      StorageUploadTask uploadTask =
          firebaseStorageRef.putFile(profileData.imageFile);
      profileData.imageUrl =
          await (await uploadTask.onComplete).ref.getDownloadURL();
    }

    return _firestore.collection('users').document(currentUser.uid).updateData({
  ...
  List<String> _getSearchTermsByString(String source) {
    ...
    List<String> longTerms =
        source.split(' ').map((term) => term.toLowerCase()).toList();
